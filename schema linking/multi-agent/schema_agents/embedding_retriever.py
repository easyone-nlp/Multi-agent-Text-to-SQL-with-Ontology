...
class EmbeddingSchemaRetriever:
    ...
    def __init__(
        ...
        self.model = model
        self.top_k_tables = max(1, top_k_tables)
        self.top_k_columns = max(1, top_k_columns)
    ...
    def retrieve(
        ...
        if self.strict_top_k:
            allowed_tables = set(top_tables)
            candidate_columns = [
                (key, vector)
            ...
            top_columns = _top_keys(
                query_vector,
                [key for key, _ in candidate_columns],
                [vector for _, vector in candidate_columns],
                self.top_k_columns,
            )

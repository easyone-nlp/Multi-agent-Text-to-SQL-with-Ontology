...
def extract_gold_links(
    ...
    *,
    ...
    try:
        return _extract_gold_links_with_sqlglot(schema, sql)
    except ImportError as error:
        if require_sqlglot:
            raise RuntimeError(
                "gold schema 추출에는 sqlglot이 필요합니다. "
                "현재 실험 환경에 sqlglot을 설치하세요."
            ) from error
    except Exception as error:
        if require_sqlglot:
            raise RuntimeError(
                f"sqlglot으로 gold SQL schema를 추출하지 못했습니다: {error}"
            ) from error
    return _extract_gold_links_with_identifiers(schema, sql)


def _extract_gold_links_with_sqlglot(
    ...
    table_names = {table.casefold(): table for table in schema.tables}
    columns_by_table: dict[str, dict[str, str]] = {}
    for column in schema.columns:
        columns_by_table.setdefault(column.table, {})[
    ...
    for table_expression in expression.find_all(exp.Table):
        table = table_names.get(table_expression.name.casefold())
        if table is not None:
            gold_tables.add(table)
            global_sources[table_expression.alias_or_name.casefold()] = table
            global_sources[table_expression.name.casefold()] = table
    ...
    for scope in traverse_scope(expression):
        ...
        for alias, source in scope.sources.items():
            if not isinstance(source, exp.Table):
                # A derived table or subquery is resolved in its own scope.
                continue
            table = table_names.get(source.name.casefold())
            if table is None:
                continue
            local_sources[str(alias).casefold()] = table
            local_sources[source.name.casefold()] = table

        local_tables = set(local_sources.values())
        for column_expression in scope.columns:
            column_name = column_expression.name
            if not column_name or column_name == "*":
                continue
            normalized_name = column_name.casefold()
            qualifier = column_expression.table
            if qualifier:
                table = local_sources.get(qualifier.casefold())
                if table is None:
                    table = table_names.get(qualifier.casefold())
                if table is not None:
                    key = columns_by_table.get(table, {}).get(normalized_name)
                    if key is not None:
                        gold_columns.add(key)
                continue

            candidates = {
                columns_by_table[table][normalized_name]
                for table in local_tables
                if normalized_name in columns_by_table.get(table, {})
            }
            if len(candidates) == 1:
                gold_columns.update(candidates)
                continue

            # Correlated columns can be unqualified and belong to an outer scope.
            global_candidates = {
                columns_by_table[table][normalized_name]
            ...
            if len(global_candidates) == 1:
                gold_columns.update(global_candidates)

    # Some sqlglot scopes omit columns nested in HAVING aggregates. Walk every
    # Column node once more and resolve only unambiguous physical-schema names.
    for column_expression in expression.find_all(exp.Column):
        ...
        qualifier = column_expression.table
        if qualifier:
            table = global_sources.get(qualifier.casefold())
            if table is None:
                table = table_names.get(qualifier.casefold())
            if table is not None:
                key = columns_by_table.get(table, {}).get(normalized_name)
...
def _extract_gold_links_with_identifiers(
    schema: DatabaseSchema, sql: str
) -> tuple[set[str], set[str]]:

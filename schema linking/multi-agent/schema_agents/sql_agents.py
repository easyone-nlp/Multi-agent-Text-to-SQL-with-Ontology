...
def extract_sql(response: str) -> str:
    ...
    match = re.search(r"\b(?:SELECT|WITH)\b", stripped, flags=re.I)
    if not match:
        raise ValueError("model response에 SELECT/WITH SQL이 없음")
    sql = _first_sql_statement(stripped[match.start():])
...
def safe_select_error(sql: str) -> str | None:
    ...
    if _first_sql_statement(without_trailing) != without_trailing.strip():

...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use only table/column identifiers extracted from gold SQL, then run "
            "the existing SQL generation and evaluation pipeline"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Use Qwen semantic pruning, embedding candidates, the official "
            "multi-source DANKE retriever, or the separate DANKE+embedding hybrid"
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help="Cap retrieval to table top-k and column top-k within those tables",
    )
    parser.add_argument(
        "--danke-use-steiner",
        action=argparse.BooleanOptionalAction,
        default=None,
        help=(
            "Enable or disable DANKE Steiner bridge/FK hints without changing "
            "the selected retriever or Join Linker"
        ),
    )
...
def main() -> None:
    ...
    if args.gold_schema and args.linking_input:
        raise SystemExit("--gold-schema와 --linking-input은 함께 사용할 수 없습니다.")
    if args.gold_schema and args.question:
        raise SystemExit("--gold-schema는 gold SQL이 있는 dataset 예제에서만 사용할 수 있습니다.")
    if args.gold_schema and not args.generate_sql:
        raise SystemExit("--gold-schema 실험에는 --generate-sql도 필요합니다.")
    if (
        args.linking_input
        and args.output
    ...
    linker = (
        None
        if linking_cache is not None or args.gold_schema
    ...
    for index, example in enumerate(examples, start=args.offset):
        ...
        if args.gold_schema:
            try:
                result = build_gold_schema_result(example, schema)
            except (RuntimeError, ValueError) as error:
                raise SystemExit(
                    f"gold schema 구성 실패: index={index}, db_id={example.db_id}: {error}"
                ) from error
        elif linking_cache is not None:
        ...
        if args.gold_schema:
            record["schema_linking_source"] = {
                "mode": "gold_schema",
                "derived_from": "gold_sql_identifiers_only",
                "gold_sql_exposed_to_sql_generator": False,
            }
    ...
    summary: dict[str, Any] = {
        ...
        "retrieval_mean_table_recall": _mean_or_none(
            retrieval_table_recalls
        ),
        "retrieval_strict_table_recall": _mean_or_none(
            strict_retrieval_table_recalls
        ),
        "retrieval_mean_column_recall": _mean_or_none(
            retrieval_column_recalls
        ),
        "retrieval_strict_column_recall": _mean_or_none(
            strict_retrieval_column_recalls
        ),
    }
    if args.gold_schema:
        summary["schema_linker_mode"] = "gold_schema"
        summary["gold_schema_source"] = "gold_sql_identifiers_only"
        summary["gold_sql_exposed_to_sql_generator"] = False
    elif linker is not None:
        summary["schema_linker_mode"] = linker.schema_linker_mode
        summary["retrieval_mode"] = (
            "strict" if linker.retriever.strict_top_k else "expanded"
        )
        summary["retrieval_top_k_tables"] = linker.retriever.top_k_tables
        summary["retrieval_top_k_columns"] = linker.retriever.top_k_columns
        danke_retriever = getattr(
            linker.retriever, "danke", linker.retriever
        )
        if hasattr(danke_retriever, "use_steiner"):
            summary["retrieval_use_steiner"] = bool(
                danke_retriever.use_steiner
            )
...
def _linking_config(config: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    embedding_overrides = {
        ...
        "strict_top_k": args.strict_retrieval_top_k,
    }
    for key, value in embedding_overrides.items():
        if value is not None:
            embedding_config[key] = value
    linking_config["embedding"] = embedding_config
    danke_config = dict(linking_config.get("danke", {}))
    if args.danke_use_steiner is not None:
        danke_config["use_steiner"] = args.danke_use_steiner
    linking_config["danke"] = danke_config

...
TRAIN_TABLES_FILE = SPIDER_DATA_DIR / "Spider 1.0" / "train" / "train_tables.json"
DEV_TABLES_FILE = SPIDER_DATA_DIR / "Spider 1.0" / "dev" / "dev_table.json"

SCHEMA_AUGMENTATION_DIR = PROJECT_ROOT / "schema_augmentation"
TABLE_RESULT_FILE = SCHEMA_AUGMENTATION_DIR / "table_synonyms.json"
CHECKPOINT_FILE = SCHEMA_AUGMENTATION_DIR / "table_synonyms_checkpoint.jsonl"
ERROR_FILE = SCHEMA_AUGMENTATION_DIR / (
    f"table_synonyms_errors_"
...
# 시험 실행 시 1~2로 설정 가능
# 전체 166개 DB를 처리하려면 None
MAX_DATABASES: int | None = 1
...
def readable_table_name(table_name: str) -> str:
    """테이블명을 읽기 쉽게 변환."""
    value = re.sub(
        r"(?<=[a-z0-9])(?=[A-Z])",
        " ",
...
train = read_json(TRAIN_TABLES_FILE)
dev = read_json(DEV_TABLES_FILE)
...
# ============================================================
# 5. train/dev tables에서 테이블 추출
# ============================================================

def load_table_tasks(
    ...
    if max_databases is not None:
        selected_tables = selected_tables[:max_databases]

    tasks: list[dict[str, Any]] = []

    for db in selected_tables:
        db_id = db["db_id"]

        table_names_original = db.get(
        ...
        for index, table_name in enumerate(table_names_original):
            ...
            readable_name = (
                ...
                else readable_table_name(table_name)
            )
...
주어진 데이터베이스 테이블 각각에 대해 다음을 생성하세요.

1. description
- 테이블의 비즈니스 의미를 설명하는 한국어 한 문장
- 데이터베이스와 테이블의 문맥을 반영할 것
- 불필요하게 장황하게 쓰지 말 것
...
중요 규칙:
- db_id, table은 입력값을 철자, 대소문자, 밑줄까지
  완전히 동일하게 유지하세요.
- table의 밑줄을 공백으로 바꾸지 마세요.
- table의 대소문자를 바꾸지 마세요.
    ...
    for task in task_group:
        table_lines.append(
            "\n".join(
                [
                    f"- table: {task['table']}",
                    f"- current_label: {task['readable_name']}",
                ]
            )
        )
...
def make_prompt_jobs(
    ...
    for task in tasks:
        db_id = task["db_id"]
        grouped.setdefault(db_id, []).append(task)

    jobs: list[dict[str, Any]] = []

    for db_id, db_tasks in grouped.items():
...
def validate_and_convert_response(
    ...
    테이블은 이름으로 우선 매칭하고,
    찾지 못하면 단일 테이블 요청인 경우 첫 결과를 사용한다.
    """
    try:
    ...
    for expected in expected_tasks:
        ...
        generated = generated_by_table.get(
            expected_table
        )
        ...
        if (
            ...
            generated = parsed.tables[0]

        if generated is None:
            raise ValueError(
                f"응답에서 테이블을 찾지 못했습니다: "
                f"expected={expected_table}, "
                f"actual={list(generated_by_table)}"
            )
        ...
        if not description:
            raise ValueError(
                f"{expected_table}: description이 비어 있습니다."
            )

        if not contains_korean(description):
            raise ValueError(
                f"{expected_table}: description에 "
                "한국어가 없습니다."
            )
        ...
        result = {
            ...
            "readable_name": expected["readable_name"],
...
def process_job_with_fallback(
    ...
    """
    여러 테이블 요청이 실패하면 절반으로 나눠 재시도한다.
    한 테이블까지 실패하면 오류로 기록한다.
    """
    tasks = job["tasks"]
